const max = (...args) =>
  args.reduce((acc, v) => (acc > v ? acc : v), 0);
    ...
    instances: r
      ...
      .map((inst) =>
        inst
          .trim()
          .split(",")
          .map((colorInst) => colorInst.trim().split(" "))
          .map(([num, color]) => ({ num: +num, color }))
      ),
  }));

const part1 = (input) => parseInput(input)
  .filter((g) => g.instances.filter((r) => r.filter(x => x.num <= maxCubes[x.color]).length === r.length).length === g.instances.length)
  .reduce((acc, v) => acc + v.i, 0);

const part2 = (input) => parseInput(input)
    .map((game) => ({
      game,
      maxes: game.instances.reduce(
        (acc, v) => ({
          red: max(acc.red, v.find((x) => x.color === "red")?.num ?? 0),
          green: max(acc.green, v.find((x) => x.color === "green")?.num ?? 0),
          blue: max(acc.blue, v.find((x) => x.color === "blue")?.num ?? 0),
        }),
        {
          red: 0,
          green: 0,
    ...
    .map((g) => ({ ...g, power: g.maxes.red * g.maxes.blue * g.maxes.green }))
...
const input2 = `Game 1: 3 blue, 2 green, 6 red; 17 green, 4 red, 8 blue; 2 red, 1 green, 10 blue; 1 blue, 5 green
