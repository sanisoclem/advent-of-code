...
...$.*....
.664.598..`;
const test2 = `.242......276....234............682.......................958..695..742................714......574..............833.........159....297.686.
.............*............................612*......304..*..........*.......@175...#...*...........*890...........*.............*..*........
...
const lookup = (col, row) => input[row * width + col];
const symbols = [...input.matchAll(/[^\.\d\n]/g)].map((v) => ({
  symbol: v[0],
  pos: v.index,
  col: v.index % width,
  row: Math.floor(v.index / width),
}));
const nums = [...input.matchAll(/\d+/g)]
  .map((v) => ({
    ...
    pos: v.index,
    len: v[0].length,
    colEnd: (v.index % width) + v[0].length - 1,
    col: v.index % width,
    row: Math.floor(v.index / width),
  }))
  .map((v) => ({
    ...v,
    minCol: v.col > 0 ? v.col - 1 : 0,
    maxCol: v.colEnd >= width - 2 ? v.colEnd : v.colEnd + 1,
    minRow: v.row > 0 ? v.row - 1 : v.row,
    maxRow: v.row >= height - 1 ? v.row : v.row + 1,
  }))
  .map((v) => ({
    ...v,
    neighbors: rangeInclusive(v.minRow, v.maxRow)
      .flatMap((r) =>
        rangeInclusive(v.minCol, v.maxCol).map((c) => ({
          row: r,
          col: c,
          value: lookup(c, r),
        }))
      )
      .filter((n) => n.row !== v.row || n.col < v.col || n.col > v.colEnd),
  }))
  .map((v) => ({
    ...v,
    isPartNumber: v.neighbors.find(
      (n) => n.value !== "." && isNaN(parseInt(n.value))
    ),
  }))
  .map((v) => ({
    ...v,
    isPartNumber2: symbols.find(
      (s) =>
        Math.abs(s.row - v.row) <= 1 &&
        s.col >= v.col - 1 &&
        s.col <= v.colEnd + 1
    ),
  }));
...
const gears = [...input.matchAll(/\*/g)]
  ...
  .map((v) => ({
    ...v,
    adjacentNums: nums.filter(
      (n) =>
        Math.abs(n.row - v.row) <= 1 &&
        (v.col >= n.col - 1 && v.col <= n.colEnd + 1)
    ),
  }))
  .map((v) => ({
    ...v,
    isGear: v.adjacentNums.length === 2,
    gearRatio: v.adjacentNums[0]?.num * v.adjacentNums[1]?.num,
  }));

console.log(
  nums.filter((v) => !!v.isPartNumber).reduce((acc, v) => v.num + acc, 0),
  nums.filter((v) => !!v.isPartNumber != !!v.isPartNumber2),
  gears.filter((g) => g.isGear).reduce((acc, v) => (acc + v.gearRatio), 0)
);
